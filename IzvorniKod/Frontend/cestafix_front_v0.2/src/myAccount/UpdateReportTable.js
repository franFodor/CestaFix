      ...
      {problems && (
        <div className="w-full overflow-auto flex">
          ...
          <div className="flex items-center justify-center mr-4">
            {!loading && (<button className="bg-blue-500 hover:bg-blue-700 text-white 
                                    font-bold py-2 px-4 rounded mt-4 inline-block"
              onClick={commitChanges}>Potvrdi Odabrane Promjene!</button>)}
            {loading && (<div><b>Uspješno ažurirano!</b></div>)}
            <div></div>
          </div>
        </div>
      )}
