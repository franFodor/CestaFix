...
Sastoji se od atributa: user\_id, firstname, lastname, email, password, role i citydept\_id.
...
Sastoji se od atributa: citydept\_id i citydept\_name. 
Povezan je vezom \textit{One-to-Many} s entitetom Users preko atributa citydept\_id i vezom \textit{One-to-Many} s entitetom CityDeptCategory preko atributa citydept\_id.
\begin{longtblr}[
	label=none,
	entry=none
	]{
...
Povezan je vezom \textit{One-to-Many} s entitetom Problems preko atributa category\_id i vezom \textit{One-to-Many} s entitetom CityDeptCategory preko atributa category\_id.
\begin{longtblr}[
	label=none,
	entry=none
	]{
...
Sastoji se od atributa: citydept\_id i category\_id. 
Povezan je vezom \textit{Many-to-One} s entitetom CityDept preko atributa citydept\_id i vezom \textit{Many-to-One} s entitetom Category preko atributa category\_id.
...
\begin{longtblr}[
	...
	\SetCell{LightBlue}photo\_id  	 & INT & jedinstveni identifikator slike (photo.photo\_id)         \\ \hline
	\SetCell{LightBlue}photo\_data   & BYTEA & slika oštećenja \\ \hline
	\SetCell{LightBlue}report\_id    & INT & jedinstveni identifikator prijave (report.report\_id) \\ \hline
\end{longtblr}

\noindent\textbf{Photo} je entitet koji sadrži sve bitne informacije vezane za fotografiju oštećenja.
Sastoji se od atributa: photo\_id, photo\_data i report\_id. 
Povezan je vezom \textit{Many-to-One} s entitetom Reports preko atributa report\_id.
...
\begin{longtblr}[
	...
	\hline \SetCell[c=3]{c}{\textbf{Photo}}                                                              \\ \hline[3pt]
	\SetCell{LightBlue}photo\_id  	 & INT & jedinstveni identifikator fotografije (photo.photo\_id)         \\ \hline
	\SetCell{LightBlue}photo\_data   & BYTEA & slika oštećenja \\ \hline
	\SetCell{LightBlue}report\_id    & INT & jedinstveni identifikator prijave (report.report\_id) \\ \hline
\end{longtblr}
...
\begin{figure}[H]
	\includegraphics[scale=0.50]{slike/ERD-model} %veličina slike u odnosu na originalnu datoteku i pozicija slike
	\centering
...
Također, zbog ograničenja tehnologije Spring Boot-a postoji razred CityDeptCategoryId u kojem ručno implementiramo
metode koje nisu automatski generirane zbog postojanja više od jednog atributa kao primarni ključ. Enumeracija 
Role predstavlja ulogu koju registrirani korisnik ima. Razred Role predstavlja enumeraciju uloga koje registrirani 
korisnik može imati. Razred Photo predstavlja fotografiju oštećenja koju je korisnik dodao prilikom prijave oštećenja. 
...
\begin{figure}[H]
	\includegraphics[scale=0.60]{slike/DS.jpg} %veličina slike u odnosu na originalnu datoteku i pozicija slike
	\centering
...
\begin{figure}[H]
	\includegraphics[scale=0.60]{slike/DK.jpg} %veličina slike u odnosu na originalnu datoteku i pozicija slike
	\centering
