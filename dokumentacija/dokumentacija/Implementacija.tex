\chapter{Implementacija i korisničko sučelje}
		...
		\section{Ispitivanje programskog rješenja}
			...
			Na taj nacin se testira stvarni proces i procedura koja se odvija kada korisnik pristupi tom API-u. 
			Postoji sveukupno 5 integracijskih testova - po jedan za svaki model koji postoji. 
			...
			Također, postoji  i funckija koja je anotirana sa @AfterEach anotacijom, što znači da će se ta funkcija izvršiti poslije svakog testa.
			Sami testovi su anotirani sa anotacijom @Test. Ovo su sve anotacije iz org.junit biblioteke.
			\begin{figure}[H]
			...
			Nakon što se baza digne, a prije pokretanja testova, izvršava se funckija s anotacijom @BeforeEach koja u bazu umeće podatke potrebne za testiranje. 
			...
			Na kraju se SQL naredba izvršava i podatak se upisuje u bazu (slika \ref{fig:IT_beforeEach}).
			\begin{figure}[H]
				\includegraphics[scale=0.60]{slike/IT_beforeEach.png}
				\centering
				\caption{Funkcija s anotacijom @BeforeEach}
				\label{fig:IT_beforeEach}
			\end{figure}

			Testovi su funkcije s anotacijom @Test. 
			Za testiranje se koristi mockMvc.perform() funkcija koja simulira HTTP zahtjev, definira HTTP metodu pri pozivu API-a,
			predaje JSON ako treba, definira kakav contentType treba predati, je li potreban jwt za pristup API-ju i koji HTTP status 
			se očekuje kao odgovor tog API-a (slika \ref{fig:IT_test}).
			\begin{figure}[H]
				\includegraphics[scale=0.60]{slike/IT_test.png}
				\centering
				\caption{Primjer testa}
				\label{fig:IT_test}
			\end{figure}

			Naziv samih API-a je organiziran tako da API-i imaju ili prefix `/advanced` ili `/normal` ili `/public`. 
			Tako su organizirani radi jasnijeg definiranja sigurnosti. 
			Naime, svi korisnici mogu pristupiti API-ima koji u nazivu imaju prefix `/public`. 
			Autenticirani korisnici mogu pristupiti API-ima koji u nazivu imaju prefix `/normal`. 
			Korisnici koji su gradski službenici (imaju Role.STAFF) mogu pristupiti API-ima koji u nazivu imaju prefix `/advanced`.
			...
			Na ovakav način se osigurava da ako se nešto umetne u bazu prilikom testa, poslije završetka testa će se i izbrisati (slika \ref{fig:IT_afterEach}).
			\begin{figure}[H]
				\includegraphics[scale=0.60]{slike/IT_afterEach.png}
				\centering
				\caption{Funkcija s anotacijom @AfterEach}
				\label{fig:IT_afterEach}
			\end{figure}
